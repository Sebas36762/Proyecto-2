...
def level2():
    ...
    C_level2 = Canvas(leveltwo,bg='White',width=800,height=800)
    C_level2.place(x=0,y=0)

    

    images= load_sprite('tile*.png')
    ship = C_level2.create_image(350,700, tags='ship')
    ...
    def recursive_animation(i):
        ...
        if(i==3):
            i=0
        if(ACTIVE==True):
            C_level2.itemconfig('ship',image=images[i])
            time.sleep(0.1)
            Thread(target=recursive_animation,args=(i+1,)).start()
    Thread(target=recursive_animation,args=(0,)).start()    
    ...
    def move_ship(event):
        if event.keysym =='Up':
            if C_level2.coords(ship)[1]>0:
                C_level2.move(ship,0,-13)
        elif event.keysym =='Down':
            if C_level2.coords(ship)[1]<780:
                C_level2.move(ship,0,13)
        elif event.keysym =='Left':
            if C_level2.coords(ship)[0]>30:
                C_level2.move(ship,-13,0)
        elif event.keysym =='Right':
            if C_level2.coords(ship)[0]<780:
                C_level2.move(ship,13,0)

    C_level2.bind_all('<KeyPress-Up>',move_ship)
    C_level2.bind_all('<KeyPress-Down>',move_ship)
    C_level2.bind_all('<KeyPress-Left>',move_ship)
    C_level2.bind_all('<KeyPress-Right>',move_ship)
    #C_level1.bind_all('<KeyRelease-space>',fire)
...
def level3():
    ...
    C_level3 = Canvas(levelthree,bg='White',width=800,height=800)
    C_level3.place(x=0,y=0)

    

    images= load_sprite('tile*.png')
    ship = C_level3.create_image(350,700, tags='ship')
    ...
    def recursive_animation(i):
        ...
        if(i==3):
            i=0
        if(ACTIVE==True):
            C_level3.itemconfig('ship',image=images[i])
            time.sleep(0.1)
            Thread(target=recursive_animation,args=(i+1,)).start()
    Thread(target=recursive_animation,args=(0,)).start()    
    ...
    def move_ship(event):
        if event.keysym =='Up':
            if C_level3.coords(ship)[1]>0:
                C_level3.move(ship,0,-13)
        elif event.keysym =='Down':
            if C_level3.coords(ship)[1]<780:
                C_level3.move(ship,0,13)
        elif event.keysym =='Left':
            if C_level3.coords(ship)[0]>30:
                C_level3.move(ship,-13,0)
        elif event.keysym =='Right':
            if C_level3.coords(ship)[0]<780:
                C_level3.move(ship,13,0)

    C_level3.bind_all('<KeyPress-Up>',move_ship)
    C_level3.bind_all('<KeyPress-Down>',move_ship)
    C_level3.bind_all('<KeyPress-Left>',move_ship)
    C_level3.bind_all('<KeyPress-Right>',move_ship)
    #C_level3.bind_all('<KeyRelease-space>',fire)
